...
import "../../styles/chat/EmbeddedChat.css";

function EmbeddedChat({ chatId }) {
  ...
  if (!chatId) return <p className="embedded-chat-status">No chat selected.</p>;
  if (loading) return <p className="embedded-chat-status">Loading chat...</p>;
  if (error) return <p className="embedded-chat-status">{error}</p>;
  ...
  return (
    <div className="embedded-chat">
      <div className="embedded-chat-header">
        <div className="embedded-chat-avatar">
          {otherUserName[0]?.toUpperCase() || "C"}
        </div>
        <div className="embedded-chat-header-text">
          <div className="embedded-chat-name">{otherUserName}</div>
          {isOtherTyping && <div className="embedded-chat-typing">typing…</div>}
        </div>
      </div>
        ...
        {combinedItems.map((item) => {
          ...
          if (item.type === "text") {
            return (
              <div
                key={item.id}
                className={`embedded-chat-row ${
                  isMine
                    ? "embedded-chat-row--mine"
                    : "embedded-chat-row--theirs"
                }`}
              >
                <div
                  className={`embedded-chat-bubble ${
                    isMine
                      ? "embedded-chat-bubble--mine"
                      : "embedded-chat-bubble--theirs"
                  }`}
                >
                  <div className="embedded-chat-text">{item.content}</div>
                </div>
              </div>
            );
          }
      ...
      <div className="embedded-chat-input">
        ...
        <input
          ...
          className="embedded-chat-input-field"
        />
        <button
          onClick={sendMessage}
          disabled={!connection || !text.trim()}
          className={`embedded-chat-send ${
            !connection || !text.trim() ? "embedded-chat-send--disabled" : ""
          }`}
        >
          Send
        </button>
